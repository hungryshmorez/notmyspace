...
- **Episodes**: 63 episodes across 28 shows, led by *That Time Again with Al & Sloppy* (11). They all play right on the site, in a theater player that rolls on to the next episode.
...
| `/shows/<slug>` | Every show has a page with its poster, genre, status, full synopsis and more shows from the same genre. Shows with episodes add a Play button and their episodes, and That Time Again with Al & Sloppy's page also has the cast and the sets. |
